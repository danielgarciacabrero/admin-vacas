...
const cargarMisFichajes = async () => {
  try {
    ...
    if (yo) {
      const res = await client.get('/fichajes', { params: { employee_id: yo.id } });
    ...
    <div v-else class="bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 p-8 mb-8">
      <div v-if="!fichajeAbierto" class="text-center">
        ...
        <div class="max-w-md mx-auto mb-4">
          <input v-model="comentario" type="text" placeholder="Comentario opcional (ej: entro tarde por consulta médica)"
            class="w-full border border-gray-300 p-3 rounded-xl outline-none focus:border-indigo-500 shadow-sm text-sm" />
        </div>
        <button @click="ficharEntrada" :disabled="fichando"
      ...
      <div v-else class="text-center">
        ...
        <div class="max-w-md mx-auto mb-4">
          <input v-model="comentario" type="text" placeholder="Comentario de salida (ej: salgo antes por cita médica)"
            class="w-full border border-gray-300 p-3 rounded-xl outline-none focus:border-red-500 shadow-sm text-sm" />
        </div>
        <button @click="ficharSalida" :disabled="fichando"
      ...
      <div v-if="mensaje" class="mt-6 bg-green-50 border border-green-200 text-green-700 p-3 rounded-lg text-sm text-center">{{ mensaje }}</div>
      <div v-if="mensajeError" class="mt-6 bg-red-50 border border-red-200 text-red-700 p-3 rounded-lg text-sm text-center">{{ mensajeError }}</div>
    </div>

    <div class="bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
      <div class="p-5 border-b border-gray-100"><h4 class="font-bold text-gray-700">Mi historial de fichajes</h4></div>
